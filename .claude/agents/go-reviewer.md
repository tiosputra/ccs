...
When invoked:
1. Establish the working directory as described above, then scope the review:
   `git diff <base> -- '*.go'`, or `git diff -- '*.go'` when no base ref was given.
2. Run `go vet ./...` and `staticcheck ./...` if available
3. Focus on modified `.go` files
4. If no Go changes turn up, stop and report that rather than reviewing the whole repo
5. Begin review
